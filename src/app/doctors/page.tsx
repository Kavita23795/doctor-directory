      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <form method="GET" action="/doctors" className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <Link href="/doctors" className="bg-gray-500 hover:bg-gray-600 text-white py-2 px-6 rounded-lg">
                🔄 Clear All
              </Link>
            </div>
          </form>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {doctors.map((doctor: Doctor) => (
              <div key={doctor.id} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
                ...
                {doctor.oncology && <p className="text-green-600 text-sm">{doctor.oncology}</p>}

                <div className="mt-2 text-sm text-gray-600">
                  <div><strong>Hospital:</strong> {doctor.hospital_name}</div>
                  <div><strong>Location:</strong> {doctor.hospital_city}, {doctor.state}</div>
                </div>
        ...
        {/* {totalPages > 1 && (
          <div className="mt-8 flex justify-center items-center space-x-2">
            {currentPage > 1 && (
              <Link href={buildPaginationUrl(searchParams, currentPage - 1)} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                ← Previous
              </Link>
            )}
